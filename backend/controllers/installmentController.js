...
const createInstallment = async (req, res) => {
    const{ user, gemID, planID, noOfMonths, totalAmount, initialPayment, monthlyPayment, installmentDates, status } = req.body

    //add doc to db
    try{
        // const user_id = req.user._id
        const installment = await Installment.create({ user, gemID, planID, noOfMonths, totalAmount, initialPayment, monthlyPayment, installmentDates, status })
        res.status(200).json(installment)
    } catch (error) {
